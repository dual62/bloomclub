...
export default function BloomieChat() {
  ...
  useEffect(() => {
    if (typing) endRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [typing])

  useEffect(() => {
    const lastMsg = messages[messages.length - 1]
    if (lastMsg?.role === 'bot') {
      setTimeout(() => lastBotRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100)
    }
  }, [messages])
  ...
  const send = async (text: string) => {
    ...
    try {
      // Try AI first
      const res = await fetch('/api/bloomie', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: text, products }),
      })
      ...
      if (res.ok) {
        ...
        // Match recommended slugs to actual products
        const matchedProducts = data.recommendedSlugs
          ?.map((slug: string) => products.find(p => p.slug === slug))
          .filter(Boolean) || []

        setMessages(p => [...p, { role: 'bot', text: data.text, products: matchedProducts, time }])
        setTyping(false)
        return
      }
    } catch (e) {
      console.log('AI unavailable, falling back to keyword matching')
    }
      ...
      <div className="bg-gradient-to-br from-navy to-navy-light px-5 py-4 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1">
          <div className="text-white font-bold text-base flex items-center gap-2">
            ...
            <span className="text-[10px] font-normal text-white/40 bg-white/10 px-2 py-0.5 rounded-full">AI-powered</span>
          </div>
        ...
        <div className="max-w-2xl mx-auto flex flex-col gap-4">
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center py-10 animate-fade-up">
              ...
              <p className="text-sm text-text-soft text-center max-w-sm mb-2">Jouw persoonlijke AI-adviseur voor schoonheid & welzijn.</p>
              <p className="text-xs text-text-faint text-center max-w-sm mb-6">Vertel me waar je mee worstelt — ik begrijp je en help je de juiste producten te vinden.</p>
              <div className="flex flex-col gap-2 w-full max-w-sm">
                {[
                  { t: 'Ik heb last van droge huid', i: '✨' },
                  { t: 'Ik lig \'s nachts veel te zweten', i: '💨' },
                  { t: 'Ik voel me de hele dag moe en futloos', i: '⚡' },
                  { t: 'Mijn haar wordt steeds dunner', i: '💫' },
                  { t: 'Wat helpt tegen stemmingswisselingen?', i: '🌙' },
                ].map((s, i) => (
            ...
            <div className="flex gap-2 items-end">
              ...
              <div className="bg-white px-4 py-3 rounded-2xl rounded-bl-sm shadow-sm">
                <div className="flex gap-1.5 items-center">
                  ...
                  <span className="text-[10px] text-text-faint ml-2">Bloomie denkt na...</span>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="px-4 py-3 bg-white border-t border-navy/5 flex-shrink-0">
        <div className="max-w-2xl mx-auto flex gap-2.5">
          <input value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Stel je vraag aan Bloomie..."
        ...
        <p className="text-center text-[10px] text-text-faint mt-1.5">Bloomie biedt productadvies, geen medisch advies. Raadpleeg bij klachten altijd een arts.</p>
      </div>
    </div>
  )
}
